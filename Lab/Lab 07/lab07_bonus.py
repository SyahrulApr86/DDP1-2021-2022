...
while True:
    ...
    # jika diberikan perintah TUJUAN_KELAS_TERMURAH <tujuan_akhir> <kelas_kereta>
    elif lst_perintah[0] == "TUJUAN_KELAS_TERMURAH" and len(lst_perintah) == 3:
        # menambah dictionary ke dalam list
        if lst_perintah[2] == "Eksekutif":
            lst_perintah.append({"kelas": "1"})
        elif lst_perintah[2] == "Bisnis":
        ...
        # looping untuk menentukan tiket termurah
        harga = int(jadwal[1][3])
        out = ""
        for kereta in jadwal:
            if lst_perintah[1] == kereta[1] and kereta[0][0] == lst_perintah[3]["kelas"] and int(kereta[3]) < harga:
                harga = int(kereta[3])
                out = f"KA {kereta[0]} berangkat pukul {kereta[2]} dengan harga tiket {kereta[3]} \n"
    ...
    # jika diberikan perintah TUJUAN_JAM_TERMURAH <tujuan_akhir> <jam_keberangkatan>
    elif lst_perintah[0] == "TUJUAN_JAM_TERMURAH" and len(lst_perintah) == 3:
        # looping untuk menentukan tiket termurah
        harga = int(jadwal[1][3])
        out = ""
        for kereta in jadwal:
            if lst_perintah[1] == kereta[1] and int(kereta[2]) <= int(lst_perintah[2]) and int(kereta[3]) < harga:
                harga = int(kereta[3])
                out = f"KA {kereta[0]} berangkat pukul {kereta[2]} dengan harga tiket {kereta[3]}\n"
