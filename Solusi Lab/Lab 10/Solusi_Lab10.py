...
class WindowBeliBarang(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        self.lbl_nama_barang.grid(row = 1, column = 0)
        self.lbl_jumlah.grid(row = 2, column = 0)
    ...
    def beli_barang(self):
        ...
        if nama_barang == "":
            if not (tkmsg.askretrycancel("BarangNotFound", "Nama barang tidak boleh kosong.")):
                self.destroy()
        elif nama_barang not in self.product_dict:
            if not tkmsg.askretrycancel("BarangNotFound", f"Barang dengan nama {nama_barang} tidak ditemukan dalam BakungLapak."):
                self.destroy()
...
class WindowCheckOut(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        if len(self.daftar_dibeli) == 0 :
            tk.Label(self, \
                     text = "Belum ada barang yang dibeli :(").grid(row = i, column= 1)
            i += 1
            pass
        else :
            for barang in self.daftar_dibeli:
                tk.Label(self, text=f"{barang.get_nama()}").grid(row = i, column= 0)
                tk.Label(self, text=f"{barang.get_harga()}").grid(row = i, column= 1)
                tk.Label(self, text=f"{self.daftar_dibeli[barang]}").grid(row = i, column= 2)
                i += 1
